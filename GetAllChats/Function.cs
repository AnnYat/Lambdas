...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
    {
        ...
        if (chats.Any(x => x.ChatId == lastId))
        {
            chats = chats.SkipWhile(x => x.ChatId == lastId).Take(pageSize).ToList();
        }
        else
        {
            chats = chats.Take(pageSize).ToList();
        }

        var result = chats.Select(x => new GetAllChatsResponseItem(x) 
            {User1 = await _context.LoadAsync<User>(x.user1), 
                User2 = await _context.LoadAsync<User>(x.user2) }).ToList();
